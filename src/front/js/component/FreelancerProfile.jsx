...
const FreelancerProfile = () => {
  ...
  const [profileImage, setProfileImage] = useState(undefinedImg);
  const [completedProjects, setCompletedProjects] = useState([]);
  const [sentProposals, setSentProposals] = useState([]);
  const [stats, setStats] = useState({ offers: 0, proposals: 0, completed: 0, rating: 0 });
    ...
    const loadProfileData = async () => {
      try {
        ...
        if (profileRes.success) {
          ...
          setProfileImage(profileRes.profile.profile_picture || undefinedImg);
        } else {
          setProfileNotFound(true);
          return navigate("/freelancerForm");
        }
        ...
        const [
          ...
          actions.getFreelancerCompletedProposals(userId),
          actions.getFreelancerSentProposals(userId),
          actions.getCompletedProjects(),
          actions.getEmployerStats()
        ]);

        if (completedPropsRes.success) setCompletedProposals(completedPropsRes.proposals);
        if (sentPropsRes.success) setSentProposals(sentPropsRes.proposals);
        if (completedProjectsRes.success) setCompletedProjects(completedProjectsRes.projects);
    ...
    try {
      ...
      if (resp.success) {
        setCompletedProposals((prev) =>
          prev.map((p) => (p.id === currentProposal.id ? { ...p, reviewed: true } : p))
        );
        setProfile((prev) => ({ ...prev, rating: resp.data.new_average }));
        setModalOpen(false);
        await Swal.fire({
          icon: "success",
      ...
      <div className="row mb-5">
          ...
          <div className="card shadow-lg border-0 rounded-4 overflow-hidden">
            <div className="row g-0">
              <div className="col-md-4 bg-light text-md-start text-center py-4 px-3">
                ...
                <div className="ps-3">
                  <p className="mb-2 text-dark"><strong>Idiomas:</strong><br /> {profile.language}</p>
                  <p className="mb-2 text-dark"><strong>Tarifa por hora:</strong><br /> ${profile.hourly_rate || "No definida"}</p>
                  <div className="mb-2">
                    <strong>Habilidades:</strong><br />
                    {profile.skills.length
                      ? profile.skills.map((s) => (
                        <span key={s.id} className="badge bg-dark me-1">{s.name}</span>
                      ))
                      : <span className="badge bg-secondary">No hay habilidades</span>
                    }
                  </div>
                  <p className="mb-2 text-dark"><strong>Educación:</strong><br />{profile.education}</p>
                </div>
              </div>
                ...
                {profile.reviews.map((r) => (
                  <li key={r.id} className="list-group-item">
                    <div className="d-flex align-items-center">
                      <img
                        src={r.reviewer.profile_picture}
        ...
        <div className="col-auto d-flex flex-column gap-3 align-items-end">
          {[{ label: "Solicitudes Enviadas", value: sentProposals.length }, 
            ...
            .map((stat, index) => (
            <div key={index} className="card text-center" style={{ width: '18vh', height: '18vh' }}>
